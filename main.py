...
st.caption(
    "Forecast traffic → RTA submits → job closes → revenue from ranking improvements, "
    "rollouts, sitewide effects, and section-level RTA rates."
)
...
with st.expander("❓ Quick FAQ — What this is & how to use it"):
    st.markdown(
        """
**What it does**  
Estimates *traffic → RTA submits → job closes → revenue* from SEO ranking improvements, factoring in rollout phases, a sitewide classifier, a runway delay, and a post-runway realization ramp.

**How to use**  
1) Upload **Keyword CSV** with columns: **URL, SV, Current Rank, Page Template**.  
2) *(Optional)* Upload a **Master URL list** (one URL per row) to drive migration % off your **true page inventory**.  
3) Choose grouping (**Site Section** or **Page Template**), enter **RTA rates**, **close rate**, and **revenue bounds**.  
4) Define **rollout phases**.  
5) Set **runway** (3/6 months) and **ramp** (Step/Linear/S-curve).  
6) Tune **dampening** + **classifier caps (30/50/70/90%)**.

**Runway vs Ramp**  
- *Runway* = no incremental gains until it ends.  
- *Ramp* = how gains realize **after** runway (Step=instant, Linear=steady, S-curve=slow→fast→plateau).
...
@st.cache_data(show_spinner=False)
def load_kw_csv(file_bytes: bytes) -> pd.DataFrame:
...
def expand_rollout_phases(phases: List[Dict], groups: List[str], horizon: int, start_offset: int) -> pd.DataFrame:
    ...
    live_month = {g: horizon + 1 for g in groups}
...
# --------- Core forecast ---------
@st.cache_data(show_spinner=True)  # IMPORTANT: no persist=True
def run_forecast(
    data: pd.DataFrame,
...
# -----------------------------
# Uploads & Sidebar Controls
# -----------------------------
u1, u2 = st.columns([1, 1])
with u1:
    kw_upload = st.file_uploader(
        "Keyword CSV (URL, SV, Current Rank, Page Template)", type=["csv"], help="Required."
    )
with u2:
    master_upload = st.file_uploader(
        "Master URL list (optional)", type=["csv"], help="One URL per row; used for migration %."
    )
...
with st.sidebar:
    ...
    ramp_mode = st.radio(
        "Post-runway ramp", ["Step", "Linear", "S-curve"], index=1, horizontal=True
    )
...
group_choice = st.radio(
    "Group by", ["Site Section", "Page Template"], index=0, horizontal=True
)
...
for i in range(1, max_phases + 1):
    with phase_container.expander(f"Phase {i}", expanded=True if i == 1 else False):
        ...
        months = cols[1].number_input(
            ...
            min_value=0,
            max_value=12,
            value=3 if i <= 2 else 0,
            step=1,
            key=f"phase_months_{i}",
        )
...
# Ensure numeric dtypes for charts/totals
num_cols = [
...
summary_allin = monthly.groupby("Scenario").agg(
    ...
    RTA_allin=("RTA_allin","sum"),
    Jobs_allin=("Jobs_allin","sum"),
    RevenueMin_allin=("RevenueMin_allin","sum"),
    RevenueAvg_allin=("RevenueAvg_allin","sum"),
    RevenueMax_allin=("RevenueMax_allin","sum"),
).reset_index()
for c in ["RTA_allin","Jobs_allin"]:
    summary_allin[c] = summary_allin[c].round(0).astype(int)
...
summary_incr = monthly.groupby("Scenario").agg(
    Clicks_incr=("Clicks_incr","sum"),
    RTA_incr=("RTA_incr","sum"),
    Jobs_incr=("Jobs_incr","sum"),
    RevMin_incr=("RevMin_incr","sum"),
    RevAvg_incr=("RevAvg_incr","sum"),
    RevMax_incr=("RevMax_incr","sum"),
).reset_index()
for c in ["RTA_incr","Jobs_incr"]:
    summary_incr[c] = summary_incr[c].round(0).astype(int)
